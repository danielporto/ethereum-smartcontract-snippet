...
contract QuickSort {
    ...
    event PrintConfirmation(string, bytes32); //helps identifying the original request
    event PrintConfirmationDebug(string, uint256, bytes32, uint256[]); //trx_id, array_size, hash, elements(list)
    ...
    */
    ...
    /*
    * receives an integer and generate an array in reverse order to be sorted and simulate CPU load
    * as in cpuheavy.sol in blockbench.
    * it emits the address of the transaction
    */

    function sort(uint size, string calldata id) public {
        ...
        insert(hash, size);
        emit PrintConfirmation(id, hash);
    }


    /*
    * verbose version of sort.
    */
    function debugSort(uint size, string calldata id) public {
        ...
        bytes32 hash = keccak256(abi.encode(data));
        insert(hash,size);
        emit PrintConfirmationDebug(id, size, hash , data);
