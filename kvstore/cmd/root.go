...
func Log(message string, a ...interface{}) {
	...
	fmt.Printf("%v|%v [KVStoreClient]: %v\n", now_date, now_ts_ms, m)
}
...
func LogWtag(tag, message string, a ...interface{}) {
	...
	now_ts_ms := now.UnixNano() / 1_000_000
	now_date := now.In(location).Format("2006-01-02 15:04:05")
	fmt.Printf("%v|%v [KVStoreClient]: %v - %v\n", now_date, now_ts_ms, tag, m)
}
...
var rootCmd = &cobra.Command{
	Use:   "kvstore",
	Short: "A simple smartcontract to maintain a kvstore key value store",
...
For example:
kvstore workload --operation kvstore --host 192.168.10.166 --port 22000 \
...
func initConfig() {
	if cfgFile != "" {
		...
		// Search config in home directory with name ".kvstore" (without extension).
