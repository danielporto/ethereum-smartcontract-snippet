...
/**********************************************************************************************************************
* The kvstore workload installs a single contract and issue operations read/write (kvstore)
...
func workloadKVStore() {
	...
	for i := 0; i < threads; i++ {
		wg.Add(1)
		go kvstore(privateKey, instance, gasPrice, nonceStream, &wg, i)
	}
	wg.Wait()
...
/**********************************************************************************************************************
* The mixed workload alternates with operations that install smartcontracts and also execute kvstore ops.
***********************************************************************************************************************/
...
func workloadMixed() {
	...
	// 1. Initialize a connection
	url := "ws://" + host + ":" + port
	Log("Running mix deploy/kvstore workload")
	Log("Connecting to ethereum network: %v", url)
	...
	// use the contract to run the kvstore workload
	go generateNonceAtRate(client, fromAddress, count, duration, nonceStream, maxrate)
	for i := 0; i < threads; i++ {
		wg.Add(1)
